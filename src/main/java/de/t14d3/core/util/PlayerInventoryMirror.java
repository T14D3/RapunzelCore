...
public class PlayerInventoryMirror {
     ...
     * Slots 9-35: Target's Main Inventory (PlayerInventory slots 9-35)
    ...
    private void startSyncTask() {
        ...
        }.runTaskTimer(Main.getInstance(), 5L, 5L); // Initial delay of 5 ticks, then repeat every 5 ticks
    }
    ...
    public void handleInventoryClick(InventoryClickEvent event) {
        ...
        if (!event.getWhoClicked().hasPermission("core.invsee.modify")) {
            return;
        }
    ...
    private int getTargetInventorySlot(int displaySlot) {
        if (displaySlot >= 0 && displaySlot < 36) {
            // Main inventory and hotbar (0-35) map directly
            return displaySlot;
        } else {
