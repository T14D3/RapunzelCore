...
/**
 * Command to grant access to a pet.
 */
public class PetAccessCommand {
    ...
    public PetAccessCommand(RapunzelPaperCore core, PetModule module) {
        this.core = core;
        this.module = module;
    ...
    /**
     * Grants a player access to a pet.
     * @param owner the pet owner
     * @param targetPlayerName the player to grant access to
     * @param petName optional pet name (uses nearest if null)
     */
    public void grantAccess(Player owner, String targetPlayerName, String petName) {
        // Find target player
        ...
        if (targetUuid.equals(owner.getUniqueId())) {
            owner.sendMessage(core.getMessageHandler().getMessage("pets.access.self"));
            return;
        }
        ...
        if (repository.isAllowed(pet, targetUuid)) {
            ...
            owner.sendMessage(core.getMessageHandler().getMessage("pets.access.already", targetName));
            return;
        }

        // Grant access
        repository.addAllowedPlayer(pet, targetUuid);

        // Notify players
        String petDisplayName = pet.getCustomName() != null ? pet.getCustomName() : formatEntityType(pet.getEntityType());
        String targetName = Bukkit.getOfflinePlayer(targetUuid).getName();
        
        owner.sendMessage(core.getMessageHandler().getMessage("pets.access.granted", targetName, petDisplayName));

        // Notify target if online
        Player target = Bukkit.getPlayer(targetUuid);
        if (target != null && target.isOnline()) {
            target.sendMessage(core.getMessageHandler().getMessage("pets.access.notify_granted",
